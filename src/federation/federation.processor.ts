...
export class FederationProcessor extends WorkerHost {
    ...
    async process(job: Job<AnyFederationMessage>): Promise<any> {
        ...
        this.logger.debug(`Processing outgoing federation message of type ${message.type} to ${message.targetFederation}`);
        try {
            this.logger.log(`Attempt ${job.attemptsMade + 1}: Sending message to ${message.targetFederation}/federation/receive`);
            const response = await axios.post(
                ...
                message,
                {
                    headers: {
                        ...
                        'X-Federation-Uri': this.configService.get<string>('INSTANCE_ADDR')!
                    },
                    timeout: 10000
                }
            );
