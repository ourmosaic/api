...
export class FriendshipService {
    ...
    async sendFriendRequest(sender: UserType, dto: SendRequestDto) {
        ...
        if (existingRelation) {
            throw new BadRequestException(errorCodes.FRIENDSHIP_ALREADY_EXISTS_OR_PENDING);
        }
    ...
    async respondToFriendRequest(user: UserType, requestId: string, accept: boolean) {
        const request = await this.prisma.friendship.findUnique({ where: { id: requestId } });
        if (!request || request.userTwoId !== user.id || request.status !== FriendshipStatus.PENDING) {
        ...
        if (!accept) {
            ...
            await this.redis.publish(`user:${request.userOneId}:friendRequests`, JSON.stringify({ type: REDIS_EVENTS.FRIENDSHIP_REMOVED, userId: user.id }));
        ...
        await this.redis.publish(`user:${request.userOneId}:friendRequests`, JSON.stringify({ type: REDIS_EVENTS.FRIENDSHIP_REQUEST_UPDATED, requestId, status: newStatus }));
    ...
    async thoughtWeWereFriends(user: UserType, friendId: string) {
        ...
        await this.prisma.friendship.deleteMany({ where: { OR: friendship.map((f) => ({ id: f.id })) } });
        await this.redis.publish(`user:${friendId}:friendRequests`, JSON.stringify({ type: REDIS_EVENTS.FRIENDSHIP_REMOVED, userId: user.id }));
    ...
    async getFriends(user: UserType) {
        ...
        const friendIds = friendships.map((f) => (f.userOneId === user.id ? f.userTwoId : f.userOneId));
        return this.prisma.user.findMany({ where: { id: { in: friendIds } }, select: { id: true, username: true, system: true, isSystem: true } });
    }

    async updateFriendshipType(user: UserType, friendId: string, type: FriendshipType) {
        ...
        await this.redis.publish(`user:${friendId}:friendRequests`, JSON.stringify({ type: REDIS_EVENTS.FRIENDSHIP_UPDATED, userId: user.id, friendshipType: type }));
